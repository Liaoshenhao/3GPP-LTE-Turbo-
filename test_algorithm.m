%****************************************************************
% 内容概述：TURBO译码性能之译码算法AWGN信道测试（菜单式）
%          达到误帧限即可停止当前SNR点的测试，节省计算量
%          1024交织长度，5迭代次数，20误帧限
...
algorithm = input('译码算法【1:LOG-MAP,2:MAX-LOG-MAP,3:MAP(缺省),4:SOVA】:');
...
if save_mat==1
    matFileName = input('MAT文件名 【''data3.mat''】:');
    if isempty(matFileName)
        matFileName='data3.mat';
    end
end
...
err_counter=zeros(length(algorithm),length(EbNo));        %初始化错误比特计数器
nferr= zeros(length(algorithm),length(EbNo));             %初始化错误帧计数器
ber=zeros(length(algorithm),length(EbNo));                 %初始化错误比特率

random_in=round(rand(1,length_interleave));  %随机数
[turbo_out,position]=encoder(random_in);      %编码

for ii=1:length(algorithm) %译码算法
    for nEN=1:length(EbNo) %EbNo的采样点
        ...
        if nEN==1 || ber(ii,nEN-1)>9.0e-6
            while nferr(ii,nEN)<ferrlim        %nferr:当前迭代次数、EbNo点的错误帧数
                ...
                [hard_out,soft_out]=decoder_all_algorithm(soft_in,position,iteration,algorithm(ii),L_c); %译码
            ...
            ber(ii,nEN) = err_counter(ii,nEN)/nframe/(length_interleave);%误比特率
        else
            ber(ii,nEN)=NaN;
        end
        fprintf('译码算法：%1.0f；EbNo：%1.2fdB；误码率：%8.4e；\n',...
            algorithm(ii),EbNo(nEN),ber(ii,nEN));
        if save_mat==1
...
title('3GPP标准 5迭代次数,1024交织长度,1/3码率');
legend('Log-MAP','Max-Log-MAP','SOVA');

time_end=datestr(now);

fprintf('------------------恭喜你！测试完成！--------------------\n'); 
disp([' 仿真起始时间:',time_begin,'=>',time_end])
fprintf(' 交织长度=%4dbit；译码算法=%2d %2d %2d\n',length_interleave,algorithm(1),algorithm(2),algorithm(3));
