%****************************************************************
% 内容概述：TURBO译码性能之迭代次数AWGN信道测试（菜单式）
%          达到误帧限即可停止当前SNR点的测试，节省计算量
%          MAX-LOG-MAP算法，1024交织长度，20误帧限
...
if save_mat==1
    matFileName = input('MAT文件名 【''data1.mat''】:');
...
err_counter=zeros(length(iteration),length(EbNo));        %初始化错误比特计数器
nferr= zeros(length(iteration),length(EbNo));             %初始化错误帧计数器
ber=zeros(length(iteration),length(EbNo));                 %初始化错误比特率

random_in=round(rand(1,length_interleave));  %随机数
[turbo_out,position]=encoder(random_in);      %编码

for ii=1:length(iteration) %迭代次数
    for nEN=1:length(EbNo) %EbNo的采样点
        ...
        if nEN==1 || ber(ii,nEN-1)>9.0e-6
            while nferr(ii,nEN)<ferrlim        %nferr:当前迭代次数、EbNo点的错误帧数
                ...
                [hard_out,soft_out]=decoder_all_algorithm(soft_in,position,iteration(ii),algorithm,L_c); %译码
